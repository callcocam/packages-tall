<div class="hidden sm:block ml-3">
    <x-modal.card title="{{ __('Filtros columns') }} - {{ $model->name }}" blur="md" squared
                                ...
                                @if ($columns = array_filter($this->columns))
                                    <ul role="list" class="font-medium text-gray-900 px-2 py-3 space-y-2">
                                        @foreach ($columns as $column)
                                            @if (is_string($column))
                                                @if (!in_array($column, ['deleted_at']))
                                                    @if (!\Str::contains($column, 'able_type'))
                                                        @if (!\Str::contains($column, 'able_id'))
                                                            @if (!\Str::contains($column, '_id'))
                                                                <div
                                                                    class="flex justify-between items-center px-6 py-2 bg-gray-200">
                                                                    <span> {{ $column }}</span>
