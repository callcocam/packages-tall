                                                        ...
                                                        @if (!\Str::contains($column, 'able_id'))
                                                            @if ($name = $tableName)
                                                                @if ($options = data_get($this->localColumns, $column))
                                                                    @livewire('tall-report::admin.operacional.reports.includes.filter-component', compact('model', 'column', 'name', 'options'), key($column))
                                                                @else
                                                                    @livewire('tall-report::admin.operacional.reports.includes.filter-component', compact('model', 'column', 'name'), key($column))
                                                                @endif
                                                            @endif
                                                        @endif
                                                    @endif
                                                                ...
                                                                @if (!in_array($item, ['two_factor_secret', 'two_factor_recovery_codes', 'two_factor_confirmed_at', 'remember_token', 'deleted_at', 'current_team_id', 'email_verified_at', 'password']))
                                                                    @if (!\Str::contains($item, 'able_type'))
                                                                        @if (!\Str::contains($item, 'able_id'))
                                                                            ...
                                                                            @if ($column = $item)
                                                                                @if ($options = data_get($this->localColumns, $column))
                                                                                    @livewire('tall-report::admin.operacional.reports.includes.filter-component', compact('model', 'column', 'name', 'options'), key(sprintf('%s.%s', $name, $column)))
                                                                                @else
                                                                                    @livewire('tall-report::admin.operacional.reports.includes.filter-component', compact('model', 'column', 'name'), key(sprintf('%s.%s', $name, $column)))
                                                                                @endif
                                                                            @endif
                                                                            {{-- @endif --}}
                                                                        @endif
